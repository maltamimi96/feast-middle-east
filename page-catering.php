	...
	<main id="main-content">
		...
		<section class="section section--cream"><div class="site-wrap"><div class="page-intro"><?php the_content(); ?></div>
			...
			<?php foreach ( $bundles as $bundle ) :
				...
				?><article class="bundle-card<?php echo $featured ? ' bundle-card--featured' : ''; ?>"><?php if ( get_post_meta( $bundle->ID, '_feast_tag', true ) ) : ?><span class="bundle-tag"><?php echo esc_html( get_post_meta( $bundle->ID, '_feast_tag', true ) ); ?></span><?php endif; ?><h3><?php echo esc_html( get_the_title( $bundle ) ); ?></h3><span class="bundle-for"><?php echo esc_html( get_post_meta( $bundle->ID, '_feast_audience', true ) ); ?></span><?php if ( $features ) : ?><ul><?php foreach ( $features as $feature ) : ?><li><?php echo esc_html( $feature ); ?></li><?php endforeach; ?></ul><?php endif; ?><a class="text-link" href="<?php echo esc_url( home_url( '/contact/' ) ); ?>" data-bundle="<?php echo esc_attr( get_the_title( $bundle ) ); ?>"><?php echo esc_html( get_post_meta( $bundle->ID, '_feast_cta_label', true ) ); ?> →</a></article><?php endforeach; ?>
			</div>
		</div></section>
		...
		<?php if ( $faqs ) : ?><section class="section"><div class="site-wrap narrow-wrap"><p class="eyebrow">Good to know</p><h2>Catering questions, answered.</h2><div class="faq-list"><?php foreach ( $faqs as $faq ) : ?><details><summary><?php echo esc_html( get_the_title( $faq ) ); ?></summary><div><?php echo wp_kses_post( wpautop( $faq->post_content ) ); ?></div></details><?php endforeach; ?></div></div></section><?php endif; ?>
		<section class="section enquiry"><div class="site-wrap enquiry-grid"><div class="enquiry-copy"><p class="eyebrow"><?php echo esc_html( feast_copy( 'enquiry_eyebrow' ) ); ?></p><h2><?php echo esc_html( feast_copy( 'enquiry_title' ) ); ?></h2><p class="lead"><?php echo esc_html( feast_copy( 'enquiry_intro' ) ); ?></p><div class="contact-list"><a href="tel:<?php echo esc_attr( feast_setting( 'phone_link' ) ); ?>">Call <?php echo esc_html( feast_setting( 'phone_display' ) ); ?></a><span><?php echo nl2br( esc_html( feast_setting( 'address' ) ) ); ?></span></div></div><?php get_template_part( 'template-parts/enquiry-form' ); ?></div></section>
	</main>
	<?php
endwhile;
get_footer();
