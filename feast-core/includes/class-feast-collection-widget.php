...
class Feast_Core_Collection_Widget extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		?><div class="feast-cms-grid feast-cms-grid--<?php echo esc_attr( $s['source'] ); ?>"><?php foreach ( $items as $item ) : ?><?php if ( 'feast_gallery' === $s['source'] && ! has_post_thumbnail( $item->ID ) ) { continue; } ?><article class="feast-cms-card"><?php if ( has_post_thumbnail( $item->ID ) ) : ?><div class="feast-cms-card__image"><?php echo get_the_post_thumbnail( $item->ID, 'large', array( 'loading' => 'feast_gallery' === $s['source'] ? 'lazy' : 'eager', 'decoding' => 'async' ) ); ?></div><?php endif; ?><?php if ( 'feast_gallery' !== $s['source'] ) : ?><div class="feast-cms-card__body"><div class="feast-cms-card__title"><h3><?php echo esc_html( get_the_title( $item ) ); ?></h3><?php if ( 'yes' === $s['show_price'] && get_post_meta( $item->ID, '_feast_price', true ) ) : ?><strong><?php echo esc_html( get_post_meta( $item->ID, '_feast_price', true ) ); ?></strong><?php endif; ?></div><?php if ( 'yes' === $s['show_excerpt'] && $item->post_excerpt ) : ?><p><?php echo esc_html( $item->post_excerpt ); ?></p><?php endif; ?><?php if ( 'feast_dish' === $s['source'] && get_post_meta( $item->ID, '_feast_dietary', true ) ) : ?><small><?php echo esc_html( get_post_meta( $item->ID, '_feast_dietary', true ) ); ?></small><?php endif; ?><?php if ( 'feast_bundle' === $s['source'] ) : $features = array_filter( array_map( 'trim', preg_split( '/\r\n|\r|\n/', get_post_meta( $item->ID, '_feast_features', true ) ) ) ); if ( $features ) : ?><ul><?php foreach ( $features as $feature ) : ?><li><?php echo esc_html( $feature ); ?></li><?php endforeach; ?></ul><?php endif; endif; ?></div><?php endif; ?></article><?php endforeach; ?></div><?php
	}
}
