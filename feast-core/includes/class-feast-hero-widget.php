...
class Feast_Core_Hero_Widget extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		?><section class="hero" data-slider aria-label="Featured catering offers"><div class="hero-slides"><?php foreach ( $offers as $index => $offer ) :
			...
			?><article class="hero-slide<?php echo 0 === $index ? ' is-active' : ''; ?>" data-slide aria-hidden="<?php echo 0 === $index ? 'false' : 'true'; ?>" style="<?php echo esc_attr( $style ); ?>"><?php if ( has_post_thumbnail( $offer->ID ) ) : ?><?php echo get_the_post_thumbnail( $offer->ID, 'full', array( 'class' => 'hero-slide__image', 'alt' => '' ) ); ?><?php endif; ?><div class="site-wrap hero-content"><div class="hero-copy"><p class="eyebrow"><?php echo esc_html( get_post_meta( $offer->ID, '_feast_eyebrow', true ) ); ?></p><h1><?php echo esc_html( get_the_title( $offer ) ); ?></h1><p><?php echo esc_html( $offer->post_excerpt ); ?></p><div class="hero-actions"><?php $p_label = get_post_meta( $offer->ID, '_feast_primary_label', true ); if ( $p_label ) : ?><a class="button button--light" href="<?php echo esc_url( function_exists( 'feast_resolve_url' ) ? feast_resolve_url( get_post_meta( $offer->ID, '_feast_primary_link', true ) ) : get_post_meta( $offer->ID, '_feast_primary_link', true ) ); ?>"><?php echo esc_html( $p_label ); ?></a><?php endif; ?><?php $s_label = get_post_meta( $offer->ID, '_feast_second_label', true ); if ( $s_label ) : ?><a class="button" href="<?php echo esc_url( function_exists( 'feast_resolve_url' ) ? feast_resolve_url( get_post_meta( $offer->ID, '_feast_second_link', true ) ) : get_post_meta( $offer->ID, '_feast_second_link', true ) ); ?>"><?php echo esc_html( $s_label ); ?></a><?php endif; ?></div><?php if ( get_post_meta( $offer->ID, '_feast_note', true ) ) : ?><div class="hero-note"><?php echo esc_html( get_post_meta( $offer->ID, '_feast_note', true ) ); ?></div><?php endif; ?></div></div></article><?php endforeach; ?></div>
		<?php if ( count( $offers ) > 1 && ( 'yes' === $s['dots'] || 'yes' === $s['arrows'] ) ) : ?><div class="slider-controls"><?php if ( 'yes' === $s['dots'] ) : ?><div class="slider-dots"><?php foreach ( $offers as $index => $offer ) : ?><button class="slider-dot<?php echo 0 === $index ? ' is-active' : ''; ?>" type="button" data-slide-to="<?php echo esc_attr( $index ); ?>" aria-label="Show offer <?php echo esc_attr( $index + 1 ); ?>"<?php echo 0 === $index ? ' aria-current="true"' : ''; ?>></button><?php endforeach; ?></div><?php endif; ?><?php if ( 'yes' === $s['arrows'] ) : ?><div class="slider-arrows"><button class="slider-arrow slider-prev" type="button" data-slide-prev aria-label="Previous offer">←</button><button class="slider-arrow slider-next" type="button" data-slide-next aria-label="Next offer">→</button></div><?php endif; ?></div><?php endif; ?></section><?php
	}
}
